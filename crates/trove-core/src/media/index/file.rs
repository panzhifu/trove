...
//!
//! ```text
//! header (96 bytes)      magic, grid, cloud bounds, counts, table offset
...
use super::sort::SpatialSorter;
use crate::media::formats::types::Bounds;

/// Magic at the head of an index file.
const MAGIC: [u8; 8] = *b"TRVECLD1";
/// Version of the layout this code reads and writes.
const VERSION: u32 = 1;
/// Bytes of the fixed header.
const HEADER_BYTES: usize = 96;
/// Bytes of one chunk table entry.
const CHUNK_BYTES: usize = 40;
/// Bytes of one point record with colours.
const RECORD_COLOURS: usize = 9;
/// Bytes of one point record without.
const RECORD_PLAIN: usize = 6;

/// A batch of points, and their colours when the cloud has any.
...
pub struct IndexSummary {
    ...
    /// Whether the cloud carried colours.
    pub has_colors: bool,
}
...
pub fn build_index(
    ...
    while let Some(batch) = source.next_batch()? {
        sorter.push(&batch.points, &batch.colors)?;
    }
    let has_colors = sorter.has_colors();
    ...
    let record_bytes = if has_colors {
        RECORD_COLOURS
    } else {
        RECORD_PLAIN
    };
    ...
    let mut written = 0u64;
    {
        ...
        let mut chunk_points: Vec<[f32; 3]> = Vec::with_capacity(config.points_per_chunk);
        let mut chunk_colors: Vec<[f32; 3]> = if has_colors {
            Vec::with_capacity(config.points_per_chunk)
        } else {
            Vec::new()
        };
        let mut flush_chunk = |points: &mut Vec<[f32; 3]>,
         ...
         -> Result<(), String> {
            ...
            for (index, point) in points.iter().enumerate() {
                let mut record = [0u8; RECORD_COLOURS];
                for axis in 0..3 {
                    let fraction = if extent[axis] > 0.0 {
                        ((point[axis] - bounds.min[axis]) / extent[axis] * 65535.0).round()
                    } else {
                        0.0
                    };
        ...
        while let Some((position, color)) = sorted.next_point()? {
            chunk_points.push(position);
            if has_colors {
                chunk_colors.push(color);
            }
            if chunk_points.len() >= config.points_per_chunk {
                flush_chunk(
                    &mut chunk_points,
                    &mut chunk_colors,
                    &mut table,
...
fn decode_header(bytes: &[u8; HEADER_BYTES]) -> Result<Header, String> {
    ...
    if record_bytes != RECORD_COLOURS as u32 && record_bytes != RECORD_PLAIN as u32 {
        return Err(format!("unsupported record size {record_bytes}"));
    }
...
impl CloudIndex {
    ...
    pub fn has_colors(&self) -> bool {
        self.header.record_bytes == RECORD_COLOURS as u32
    }
    ...
    pub fn read_chunk(&self, index: usize) -> Result<PointBatch, String> {
        ...
        for index in 0..chunk.count as usize {
            ...
            if has_colors {
                colors.push([
                    record[6] as f32 / 255.0,
                    record[7] as f32 / 255.0,
                    record[8] as f32 / 255.0,
                ]);
            }
        }
        Ok(PointBatch { points, colors })
    }
}
...
mod tests {
    ...
    impl BatchSource for Batches {
        fn next_batch(&mut self) -> Result<Option<PointBatch>, String> {
            ...
            let to = (self.at + self.batch).min(self.points.len());

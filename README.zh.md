...
- **导入方式**：设置 ▸ 通用 可选「复制入库」（默认）或「链接原文件」——链接资产留在原位置，检查器带「链接」徽标并可定位原文件。

### 色标与重复文件查找
- 资产**色标**（红…紫）通过统一的色标组件设置：检查器色板行、网格右键菜单，以及网格工具栏的**按颜色筛选**下拉；智能集合也可用 `color_label` 字段匹配（含"无色标"）。
...
测试状态：`trove-core` 编译通过，**104** 个测试全部通过。`trove-app` 编译通过（2 个测试）。
...
- **trove-core** — 上述核心功能已齐并有测试覆盖（104 个测试）。

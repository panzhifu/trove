...
- 🔴 **KWin 受限 D-Bus 授权（NoAuthorized 的根因）**：KWin 只允许**桌面文件里声明了** `X-KDE-DBUS-Restricted-Interfaces=org.kde.KWin.ScreenShot2` 的应用调用截图接口（Spectacle 就是这么做的）。`packaging/linux/trove.desktop` 与 `~/.local/share/applications/trove.desktop` 已补该行；调试时可用 `KWIN_SCREENSHOT_NO_PERMISSION_CHECKS=1` 绕过授权验证捕获链路本身。注意 KWin 靠 app_id（`trove` ↔ `trove.desktop`/`StartupWMClass`）认领调用方，改完要**重启应用**。

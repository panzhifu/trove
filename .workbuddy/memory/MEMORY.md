...
- **PLY 加载性能基线（2026-09-11 实测 vs CloudCompare）**：二进制 PLY 慢 CC 1.9–3.3×（5M 点 1323 ms vs 400 ms），ASCII 反而快 25%；峰值内存是 CC 的 4–6×（5M 点 320 MB vs 75 MB）。**瓶颈不是字节解析**——1M 点的 ascii 与 binary 耗时几乎相同（258 vs 265 ms，尽管文件大 2.4×），耗时严格正比于「顶点数 × 列数」，约 44 ns/属性，根因是 `load_ply` 每顶点构造 `Vec<(String, Vec<f64>, PlyType)>`（3000 万次堆分配）+ 每属性线性扫列名。基准工具：`cargo run --release -p trove-core --example ply_bench -- --iters 5 --json FILE...`（`--mem` 测峰值），样本/对比/报告脚本在 `/home/noke/.cache/trove-scratch/ply/`，报告 `docs/ply-load-bench.html`。⚠️ 两者都**未提交**。

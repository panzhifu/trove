...
| 3 | 旋转/翻转/裁剪、JPEG 无损变换 | **已实现**（`media::edit` 纯像素变换 + `Library::batch_edit_images` 批量应用、内容寻址换 blob、缩略图/视觉指纹重算、逐资产失败报告；UI 接入编辑器对话框：旋转/翻转/裁剪（百分比坐标按每图自身尺寸解析）+ JPEG 质量，原地替换媒体文件并保留资产身份与组织关系；JPEG 走质量参数重编码，无损变换不做） | digiKam/XnView | ✅ |

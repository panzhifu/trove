...
> 调研日期：2026-09-08 ｜ 最近更新：2026-09-16（第八批核对：AVIF/JPEG-XL 缩略图已实现；PLY 点云渲染 + 顶点色、媒体宽高比预设、系统托盘、浏览器扩展 MV3/Firefox、字体三行样卡、自动更新检查已入 0.4.1/0.4.2；同步 Eagle 4.0 正式版 AI 全家桶动态）｜ 适用版本：Trove 0.4.5
>
...
**一句话总结：组织与检索内核已达到同类产品主流水准，视觉检索走无模型路线（pHash + 颜色指纹，导入自动计算、零推理依赖），三视图浏览、系统级字体管理与设置窗口化补齐；批量像素编辑（旋转/翻转/裁剪）与 XMP sidecar 导出均已前后端打通；缺口集中在「AI 自动化（需重建推理栈）、批处理出口补全（水印/标注）、对比视图」几条线上。**
...
| 7 | 系统字体浏览与管理 | **部分调整**（库内字体安装/卸载三平台齐备（Linux/macOS/Windows 每用户目录 + HKCU），检查器安装/卸载保留；2026-09-16 起系统字体浏览器整体移除（产品决策），批量「激活/停用当前全部」工具同期移除） | Eagle 字体预览/FontBase | ✅（系统字体浏览按决策移除） |
...
| 6 | XMP/IPTC 元数据写回文件或 sidecar | **已实现**（`services::xmp` 标准 packet 生成 + `Library::export_xmp_sidecars`：标题/描述/标签/评分写到媒体文件旁的 .xmp（存储 blob 或链接原文件），原子写入、全转义；不破坏原文件，无嵌入/IPTC-IIM 写回；UI 接入设置/维护入口，可批量导出全部资产的 XMP sidecar） | IMatch/ACDSee/digiKam/XnView | ✅ |
...
>
> **2026-09-16 竞品动态**：Eagle 4.0 正式版（2026-04）已发布 AI Search（以图找图 + 语义搜索，离线插件）、AI Action（命名/打标/描述/分类的 AI 批量整理工作流）、AI 模型套件（统一模型配置中心）、Eagle MCP/Skill（第三方 AI Agent 接入）、Web API v2（对齐 Plugin API 的 RESTful 全量接口 + Playground）、图片拼接工具重做（16 入 5 出、实时预览）、插件启停管理；V5 已宣布重写开发中（2026-08）。Billfish 2.5（2025-10）新增批量导入自动提取 EXIF 作标签、素材导出 PDF/ZIP。AI 线整体成为当前最大的代际差。
...
| 3 | 旋转/翻转/裁剪、JPEG 无损变换 | **已实现**（`media::edit` 纯像素变换 + `Library::batch_edit_images` 批量应用、内容寻址换 blob、缩略图/视觉指纹重算、逐资产失败报告；UI 接入编辑器对话框：旋转/翻转/裁剪（百分比坐标按每图自身尺寸解析）+ JPEG 质量，原地替换媒体文件并保留资产身份与组织关系；**链接资产写回原文件**：重编码结果原子覆盖 `extra.source_path` 指向的原文件并同步哈希/尺寸/缩略图，预览快捷编辑先弹确认（红色「写回」），批量对话框有醒目提示；JPEG 走质量参数重编码，无损变换不做） | digiKam/XnView | ✅ |
...
| 2 | 插件/脚本系统 | **骨架已实现（v1，编译期插件）**：`trove_core::plugins` 注册表 + 导入管线阶段钩子（见 `docs/PLUGINS.md`），设置 ▸ 插件可启停（`disabled_plugins`，重启生效）；内置示例插件 sidecar-notes。待做：独立 API crate、动态加载、UI 面板/预览渲染器钩子 | Eagle 插件生态（重复查找/相似图均来自插件） | P3（剩余部分） |
